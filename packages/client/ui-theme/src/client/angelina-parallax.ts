...
const modeForTheme = (themeId: string): AngelinaMode | undefined => {
...
/**
 * Owns the pointer-driven Angelina background layers and their teardown.
 * The controller only writes its namespaced body variables and DOM nodes;
 * the theme stylesheet remains responsible for positioning and visibility.
 */
export class AngelinaParallaxController {
  ...
  private readonly onPointerMove = (event: PointerEvent): void => {
    if (event.pointerType === 'touch' || this.mode === undefined || this.isReducedMotion()) return
  ...
  /** Remove all owned nodes, listeners, animation frames, and body writes. */
  dispose(): void {
    this.disable()
  ...
  private ensureRoot(): void {
    ...
    if (existing instanceof HTMLDivElement && existing.getAttribute(ROOT_OWNER_ATTRIBUTE) === 'angelina') {
      this.root = existing
      existing.setAttribute(ROOT_ATTRIBUTE, 'layers')
  ...
  private writeParallax(x: number, y: number, force = false): void {
    ...
    if (this.root === undefined || this.background === undefined || this.foreground === undefined || document.body === null) return
    ...
    const copyY = this.mode === 'light' ? y * 2.5 : y * 1
