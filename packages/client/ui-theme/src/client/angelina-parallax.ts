 ...
 * The controller only writes its namespaced body attribute and artwork nodes;
...
export class AngelinaParallaxController {
  ...
  private writeParallax(x: number, y: number, force = false): void {
    ...
    if (this.root === undefined || this.background === undefined || this.foreground === undefined) return
