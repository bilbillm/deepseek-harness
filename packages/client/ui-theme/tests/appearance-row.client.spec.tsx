...
/** AppearanceRow behavior: five previews, selection follows the persisted
...
const COPY: Record<string, string> = {
  ...
  'appearance.angelinaLight': 'Angelina Light',
  'appearance.angelinaDark': 'Angelina Dark',
}
...
describe('AppearanceRow', () => {
  it('renders five previews with the preference selected', () => {
    mount('dark')
    ...
    expect(pressed(/^Angelina Light$/)).toBe('false')
    expect(pressed(/^Angelina Dark$/)).toBe('false')
  })
  ...
  it('selects the Angelina themes through the same persisted write face', () => {
    const b = mount('system')
    fireEvent.click(screen.getByRole('button', { name: /^Angelina Dark$/ }))
    expect(b.setTheme).toHaveBeenCalledWith('angelina-dark')
  })
})
