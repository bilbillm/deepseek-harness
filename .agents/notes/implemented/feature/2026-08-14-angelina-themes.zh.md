...
Codex Dream Skin Switcher 包含两套协同设计的安洁莉娜外观：亮色的 Gravity Field 与暗色的 Midnight Gravity。它们不只是壁纸，而是分别把贯穿空会话与活跃会话的同一图片构图、可读表面、语义颜色、代码语法、滚动条和响应式人物位置组合成完整主题。DeepSeek Harness 原本只把浅色、深色与跟随系统暴露为可持久化的产品偏好；若只把一张壁纸复制到某个功能样式表，大多数表面仍会沿用默认调色板，插件加载前还会闪过中性主题，也没有成对选择与持久化约定。
...
全局 `angelina.css` 样式表拥有图片呈现。ui-layout 与 ui-conversation 为应用框架、会话栏和 composer 模式暴露与主题无关的状态标记；功能样式不会按安洁莉娜 id 分支。空会话与活跃会话使用相同的清晰 hero 图、人物位置和视差层，并保留宿主的默认 composer 位置；活跃内容额外叠加低透明度 tint、3px 浅层背景模糊与 composer-seat 渐变来保持可读性，同时保持内部文字清晰。输入框、composer、菜单、listbox 与 dialog 只在自身叶节点表面应用磨砂玻璃；应用框架和侧栏祖先不使用滤镜，使固定浮层继续相对视口定位。

四张位图资源位于主题包内，并随发布的样式产物一起复制：每套主题各一张 hero 图，另有亮色主题拆分后的后景与前景图层。`AngelinaParallaxController` 以 Codex 亮色主题原有的前景和后景幅度复刻双层指针视差。标题、选择器、composer、控件及其他界面文字在宿主布局中保持静止。暗色主题没有拆分的前景图，因此降级为单层背景的小幅移动。窗口失焦或页面隐藏时指针状态会归零，触摸输入会被忽略，窄视口和 `prefers-reduced-motion` 则渲染静止图层。
...
外观设置行提供五个响应式预览，并通过与中性偏好相同的 Host-backed 路径持久化任一安洁莉娜变体。首屏、加载外壳、空会话、活跃会话、磨砂表面、代码块和原生浏览器 chrome 共用同一个解析后的配色方案与 token 权威来源。Web bundle 包含四张主题图片、一张样式表和一个可完整清理的指针控制器；共用会话构图、资源引用、动效限制与清理行为均有测试。今后修改 Web 语义 token 集合时，必须明确同步更新两套安洁莉娜调色板。

## 测试

单元测试固定五值 settings schema、bootstrap 主题 id 与 token 交接、ThemeRuntime 持久化、五项设置预览、ThemePresenter 清理、完整 token 集合、样式表导入、四张发布图片的全部引用、空会话与活跃会话共用的图片规则、亮色主题精确图片视差幅度、静止的界面内容、暗色克制降级、减弱动效与卸载清理。类型检查与完整包构建覆盖 Host／client 分工和输出资源。浏览器验收覆盖桌面与移动端的亮暗两套主题，并检查空会话、活跃会话、设置、磨砂浮层与指针移动。
