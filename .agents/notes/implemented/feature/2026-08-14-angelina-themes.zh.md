...
全局 `angelina.css` 样式表拥有图片呈现。ui-layout 与 ui-conversation 为应用框架、会话栏和 composer 模式暴露与主题无关的状态标记；功能样式不会按安洁莉娜 id 分支。空会话使用清晰 hero 图并为左侧 composer 留出安全区域，活跃会话则使用轻度柔化、仍保留人物与环境细节的 thread 图。输入框、composer、菜单、listbox 与 dialog 只在自身叶节点表面应用磨砂玻璃；应用框架和侧栏祖先不使用滤镜，使固定浮层继续相对视口定位。

六张位图资源位于主题包内，并随发布的样式产物一起复制。`AngelinaParallaxController` 以 Codex 亮色主题原有的前景、后景和文案幅度复刻双层指针视差。暗色主题没有拆分的前景图，因此降级为单层背景的小幅移动。窗口失焦或页面隐藏时指针状态会归零，触摸输入会被忽略，窄视口和 `prefers-reduced-motion` 则渲染静止图层。
...
单元测试固定五值 settings schema、bootstrap 主题 id 与 token 交接、ThemeRuntime 持久化、五项设置预览、ThemePresenter 清理、完整 token 集合、样式表导入、六张发布图片的全部引用、亮色主题精确视差幅度、暗色克制降级、减弱动效与卸载清理。类型检查与完整包构建覆盖 Host／client 分工和输出资源。浏览器验收覆盖桌面与移动端的亮暗两套主题，并检查空会话、活跃会话、设置、磨砂浮层与指针移动。
