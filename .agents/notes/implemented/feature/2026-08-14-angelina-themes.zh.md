...
全局 `angelina.css` 样式表拥有图片呈现。ui-layout 与 ui-conversation 为应用框架、会话栏和 composer 模式暴露与主题无关的状态标记；功能样式不会按安洁莉娜 id 分支。空会话与活跃会话使用相同的清晰 hero 图、人物位置和视差层，并保留宿主的默认 composer 位置；活跃内容只额外叠加低透明度 tint 与 composer-seat 渐变来保持可读性。输入框、composer、菜单、listbox 与 dialog 只在自身叶节点表面应用磨砂玻璃；应用框架和侧栏祖先不使用滤镜，使固定浮层继续相对视口定位。
